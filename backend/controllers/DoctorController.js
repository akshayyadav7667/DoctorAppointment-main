...
export const changeAppointmentStatus = async (req, res) => {
  ...
  try {
    ...
    const doctor = await Doctor.findOne({ user_Id: doctorUserId }).populate(
      "user_Id",
      "name phone"
    );
...
export const seeAppointmentDetails = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Doctor see all Appointment ", appointment });
  } catch (error) {
    console.log(error);
    res.status(400).json({ message: error.message });
  }
...
export const doctorDashboard = async (req, res) => {
  ...
  try {
    ...
    const appointment = await Appointment.find({ doctor_Id: doctor._id })
      ...
      .sort({ createdAt: -1 });
    // console.log(appointment);
    const appovedAppointment = appointment.filter(
      (app) => app.status === "confirmed"
    ).length;
    const pendingAppointment = appointment.filter(
      (app) => app.status === "pending"
    ).length;
    const rejectAppointment = appointment.filter(
